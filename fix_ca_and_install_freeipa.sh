...
# مرحله 1: بررسی لاگ‌های خطا
print_step "1" "بررسی لاگ‌های خطا..."
echo "=== لاگ نصب FreeIPA ==="
tail -30 /var/log/ipaserver-install.log
echo ""
echo "=== لاگ PKI Tomcat ==="
tail -20 /var/log/pki/pki-tomcat/system.log 2>/dev/null || echo "فایل لاگ PKI یافت نشد"
echo ""

# مرحله 2: توقف سرویس‌ها
print_step "2" "توقف سرویس‌ها..."
...
systemctl stop pki-tomcatd@pki-tomcat 2>/dev/null || true
print_success "سرویس‌ها متوقف شدند"

# مرحله 3: حذف کامل تنظیمات
print_step "3" "حذف کامل تنظیمات..."
...
rm -rf /var/lib/pki 2>/dev/null || true
rm -rf /etc/pki 2>/dev/null || true
rm -rf /var/log/pki 2>/dev/null || true
print_success "تنظیمات حذف شدند"

# مرحله 4: حذف و نصب مجدد بسته‌ها
print_step "4" "حذف و نصب مجدد بسته‌ها..."
...
# مرحله 5: تنظیم hostname و hosts
print_step "5" "تنظیم hostname و hosts..."
...
# مرحله 8: نصب FreeIPA بدون CA
print_step "8" "نصب FreeIPA بدون CA..."
ipa-server-install \
  --domain=mci.local \
  --realm=MCI.LOCAL \
...
# مرحله 9: راه‌اندازی سرویس‌ها
print_step "9" "راه‌اندازی سرویس‌ها..."
...
# مرحله 10: صبر برای آماده شدن
print_step "10" "صبر برای آماده شدن سرویس‌ها..."
sleep 60

# مرحله 11: بررسی وضعیت
print_step "11" "بررسی وضعیت سرویس‌ها..."
systemctl status ipa --no-pager -l
echo ""

# مرحله 12: ایجاد کاربر MCI
print_step "12" "ایجاد کاربر MCI..."
ipa user-add mci \
  --first=MCI \
  --last=User \
  --email=mci@mci.local \
...
# مرحله 13: دادن دسترسی ادمین به MCI
print_step "13" "دادن دسترسی ادمین به MCI..."
ipa group-add-member admins --users=mci
print_success "دسترسی ادمین به MCI داده شد"

# مرحله 14: تست اتصال
print_step "14" "تست اتصال..."
...
# مرحله 15: بررسی پورت‌ها
print_step "15" "بررسی پورت‌ها..."
echo "=== پورت‌های در حال گوش دادن ==="
netstat -tlnp | grep -E ':(389|636|80|443|88|464)'
echo ""

# مرحله 16: نمایش اطلاعات نهایی
echo ""
echo "=========================================="
