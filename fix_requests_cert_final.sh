...
echo "=== حل نهایی مشکل Python Requests Certificate ==="
...
# مرحله 2: حل مشکل Python Requests
print_step "2" "حل مشکل Python Requests..."
...
# مرحله 3: حل مشکل Certificate Bundle
print_step "3" "حل مشکل Certificate Bundle..."
# نصب certificate های جدید
dnf install -y ca-certificates
# به‌روزرسانی certificate bundle
update-ca-trust
# ایجاد symbolic link برای certificate bundle
ln -sf /etc/pki/tls/certs/ca-bundle.crt /etc/ssl/certs/ca-bundle.crt
print_success "Certificate Bundle تنظیم شد"

# مرحله 4: بررسی فایل‌های certificate
print_step "4" "بررسی فایل‌های certificate..."
echo "=== بررسی فایل‌های certificate ==="
ls -la /etc/pki/tls/certs/ca-bundle.crt
ls -la /etc/ssl/certs/ca-bundle.crt
echo ""

# مرحله 5: تست Python Requests
print_step "5" "تست Python Requests..."
...
# مرحله 6: توقف سرویس‌ها
print_step "6" "توقف سرویس‌ها..."
...
# مرحله 7: حذف تنظیمات ناقص
print_step "7" "حذف تنظیمات ناقص..."
...
# مرحله 8: تنظیم hostname و hosts
print_step "8" "تنظیم hostname و hosts..."
...
# مرحله 11: نصب FreeIPA
print_step "11" "نصب FreeIPA..."
ipa-server-install \
  --domain=mci.local \
  --realm=MCI.LOCAL \
...
# مرحله 12: راه‌اندازی سرویس‌ها
print_step "12" "راه‌اندازی سرویس‌ها..."
...
# مرحله 13: صبر برای آماده شدن
print_step "13" "صبر برای آماده شدن سرویس‌ها..."
sleep 60

# مرحله 14: بررسی وضعیت
print_step "14" "بررسی وضعیت سرویس‌ها..."
systemctl status ipa --no-pager -l
echo ""

# مرحله 15: ایجاد کاربر MCI
print_step "15" "ایجاد کاربر MCI..."
ipa user-add mci \
  --first=MCI \
  --last=User \
  --email=mci@mci.local \
...
# مرحله 16: دادن دسترسی ادمین به MCI
print_step "16" "دادن دسترسی ادمین به MCI..."
ipa group-add-member admins --users=mci
print_success "دسترسی ادمین به MCI داده شد"

# مرحله 17: تست اتصال
print_step "17" "تست اتصال..."
...
# مرحله 18: بررسی پورت‌ها
print_step "18" "بررسی پورت‌ها..."
echo "=== پورت‌های در حال گوش دادن ==="
netstat -tlnp | grep -E ':(389|636|80|443|88|464)'
echo ""

# مرحله 19: نمایش اطلاعات نهایی
echo ""
echo "=========================================="
